...
def build_daily_sla(df: DataFrame, processing_date: str) -> DataFrame:
    ...
    delivered = (
        df
        .filter(F.col("event_type") == "ORDER_DELIVERED")
        .select(
            F.col("order_id"),
    ...
    return (
        with_sla
        .groupBy("event_date" if "event_date" in [c for c in with_sla.columns]
...
def write_gold(df: DataFrame, path: str, processing_date: str,
