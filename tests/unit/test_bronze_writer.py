...
def test_valid_events_pass_validation(spark):
    rows = [
        ("evt-001","ORDER_PLACED","ORD-001","CUST-001",
         1704067200000,1704067200000,"1.0","order-service",'{}'),
    ]
    cols = ["event_id","event_type","order_id","customer_id",
...
def test_missing_order_id_routes_to_invalid(spark):
    rows = [
        ("evt-002","ORDER_PLACED", None,"CUST-002",
         1704067200000,1704067200000,"1.0","order-service",'{}'),
    ]
    cols = ["event_id","event_type","order_id","customer_id",
            "occurred_at","ingested_at","schema_version",
    ...
    assert "missing_required_fields" in \
           invalid.select("validation_failure").first()[0]


def test_invalid_event_type_routes_to_invalid(spark):
    rows = [
        ("evt-003","UNKNOWN_TYPE","ORD-003","CUST-003",
         1704067200000,1704067200000,"1.0","order-service",'{}'),
    ]
    cols = ["event_id","event_type","order_id","customer_id",
            "occurred_at","ingested_at","schema_version",
...
def test_duplicate_event_ids_tagged(spark):
    ...
    """
    rows = [
        ("evt-dup","ORDER_PLACED","ORD-010","CUST-010",
         1704067200000, 1704067200000,"1.0","order-service",'{}'),
        ("evt-dup","ORDER_PLACED","ORD-010","CUST-010",
         1704067200000, 1704067210000,"1.0","order-service",'{}'),
    ]
    cols = ["event_id","event_type","order_id","customer_id",
            "occurred_at","ingested_at","schema_version",
            "source_system","payload"]
    df = spark.createDataFrame(rows, cols) \
              .withColumn("bronze_loaded_at", F.col("ingested_at"))
