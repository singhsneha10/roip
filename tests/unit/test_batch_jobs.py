...
SILVER_COLS = [
    ...
    "city","category","amount","item_count",
    "discount_pct","device","pincode",
    "silver_loaded_at","processing_date"
]
...
def test_hourly_orders_aggregation(spark):
    ...
    rows = [
        ("e1","ORDER_PLACED","o1","c1",
         datetime(2024,1,15,14,0),"2024-01-15",14,
         "Mumbai","electronics",1000.0,2,10.0,"android","400001",
         datetime.utcnow(),"2024-01-15"),
        ("e2","ORDER_PLACED","o2","c2",
         datetime(2024,1,15,14,30),"2024-01-15",14,
         "Mumbai","electronics",2000.0,1,0.0,"ios","400002",
         datetime.utcnow(),"2024-01-15"),
    ]
...
def test_payment_health_groups_correctly(spark):
    ...
    rows = [
        ("e3","PAYMENT_SUCCESS","o3","c3",
         datetime(2024,1,15,10,0),"2024-01-15",10,
         "Delhi","grocery",500.0,1,0.0,"web","110001",
         datetime.utcnow(),"2024-01-15"),
        ("e4","PAYMENT_FAILED","o4","c4",
         datetime(2024,1,15,11,0),"2024-01-15",11,
         "Delhi","grocery",300.0,1,0.0,"android","110002",
         datetime.utcnow(),"2024-01-15"),
        ("e5","PAYMENT_FAILED","o5","c5",
         datetime(2024,1,15,12,0),"2024-01-15",12,
         "Delhi","grocery",200.0,1,0.0,"ios","110003",
         datetime.utcnow(),"2024-01-15"),
    ]
    df     = spark.createDataFrame(rows, SILVER_COLS)
